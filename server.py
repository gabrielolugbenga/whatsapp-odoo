...
async def handle_address_response(phone: str, contact: str, address: str):
    ...
    if ambiguous:
        clarification = f"We need a bit more info for your order:\n\n"
        for amb in ambiguous:
            options = "\n".join([f"  {i+1}. {m['name']} — €{m['list_price']:.2f}" 
                                for i, m in enumerate(amb["matches"])])
            clarification += f"Which *{amb['query']}* did you mean?\n{options}\n\n"
        clarification += "Please reply with the full corrected order."
        
        await send_whatsapp(phone, clarification)
        # Keep in waiting state
        waiting_for_address[phone] = pending
        return
